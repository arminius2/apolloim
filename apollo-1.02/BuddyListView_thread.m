...
-(id) initWithFrame:(CGRect) aframe
{
	if ((self == [super initWithFrame: aframe]) != nil) 
	{
		...
		NSLog(@"Starting thread with Target:0x%08x and Object:0x%08x", [BuddyListView class], self);
		[NSThread detachNewThreadSelector:@selector(refreshThreadStart:) toTarget:[BuddyListView class] withObject:self];
	}
	return self;
}
...
+ (void)refreshThreadStart:(id)param
{
	NSLog(@"Refresh Starting with Param:0x%08x", param);
	
	NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
	while(YES)
	{
		if([param _needsRefreshing])
		{
			[param _actualReloadData];
		}

		NSLog(@"Refresh Buddy List");
