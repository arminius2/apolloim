...
static void init() {
	...
	memset(servers4,'\0',sizeof(struct in_addr) * 8);
#ifdef FIREDNS_USE_IPV6
	memset(servers6,'\0',sizeof(struct in6_addr) * 8);
#endif
...
int firedns_getmx(const char * const name) {
	...
	l = firedns_build_query_payload(name,15,1,(unsigned char *)&h.payload);
	if (l == -1)
		return -1;
	s = firedns_add_query(&h);
	if (s == NULL)
...
struct in_addr *firedns_resolveip4(const char * const name) {
	...
	return (struct in_addr *) firedns_getresult(fd);
}
...
struct in6_addr *firedns_resolveip6(const char * const name) {
	...
	FD_SET(fd,&s);
	select(fd + 1,&s,NULL,NULL,&tv);
	return (struct in6_addr *) firedns_getresult(fd);
}
