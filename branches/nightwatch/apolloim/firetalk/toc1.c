/*
toc.c - FireTalk TOC1 protocol definitions
...
enum firetalk_error toc1_save_config(client_t c) {
	...
	return toc_send_printf(c,0,"toc_set_config %s",data);
}
...
enum firetalk_error toc1_im_send_message(client_t c, const char * const dest, const char * const message, const int auto_flag) {
	...
	if (auto_flag == 1)
		return toc_send_printf(c,0,"toc_send_im %s %s auto",dest,aim_interpolate_variables(message,dest));
	else
		return toc_send_printf(c,0,"toc_send_im %s %s",dest,message);
}
...
enum firetalk_error toc1_im_send_action(client_t c, const char * const dest, const char * const message, const int auto_flag) {
	...
	if (auto_flag == 1)
		return toc_send_printf(c,0,"toc_send_im %s %s auto",dest,tempbuf);
	else
		return toc_send_printf(c,0,"toc_send_im %s %s",dest,tempbuf);
}
...
got_data_connecting_start:
	...
	switch (c->connectstate) {
		case 0: /* we're waiting for the flap version number */
			...
			c->connectstate = 1;
			r = toc_send_printf(c,1,"toc_signon login.oscar.aol.com 5190 %s %s english \"libfiretalk v" LIBFIRETALK_VERSION "\"",c->nickname,toc_hash_password(password));
			if (r != FE_SUCCESS) {
		...
		case 3:
			...
			if (c->gotconfig == 1 && c->connectstate == 3) {
				...
				if (toc1_im_upload_buddies(c) != FE_SUCCESS) {
					firetalk_callback_connectfailed(c,FE_PACKET,"Error uploading buddies");
					return FE_PACKET;
				}
				if (toc1_im_upload_denies(c) != FE_SUCCESS) {
...
enum firetalk_error toc1_subcode_send_request(client_t c, const char * const to, const char * const command, const char * const args) {
	...
	return toc1_im_send_message(c,to,buffer,0);
}
...
enum firetalk_error toc1_subcode_send_reply(client_t c, const char * const to, const char * const command, const char * const args) {
	...
	if (args == NULL)
		...
		safe_snprintf(buffer,2048,"<!--ECT %s %s-->",command,args);
	return toc1_im_send_message(c,to,buffer,1);
}
