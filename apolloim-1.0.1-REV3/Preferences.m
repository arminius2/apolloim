...
-(id) init
{
	...
	{	
		...
		if(global_prefs == nil)
		{
			AlexLog(@"Global Preference File Not Found");
		...
		{
			AlexLog(@"Global Preference File Loaded");
		}
...
//	AlexLog(@"COUNT %d", [user_dict count]);
	if([user_dict count] == 0)
	{
//		AlexLog(@"Nil... initing with file...");
...
//		AlexLog(@"Setting... %@", user_prefs);
		[user_prefs setObject: user_dict forKey: [a_user getID]];
//		AlexLog(@"Setg... %@", user_prefs);		
	}
//	AlexLog(@"COUNT %d", [user_dict count]);	
//	AlexLog(@"INITING....");
	ret = [user_dict objectForKey: a_key];
//	AlexLog(@"Reading Pref For User: %@ (%@->%@) [%@]", [a_user getStartingName], a_key, ret,  USER_PREF_PATH(a_user));
	return ret;
}

-(void) setUserPrefForUser:(User *) a_user withKey:(NSString *) a_key andValue:(NSString *) value
{
//	AlexLog(@"Setting Pref For User: %@ (%@->%@)", [a_user getStartingName], a_key, value);
	...
	if(user_dict == nil)
	{
		...
		{
			user_dict = [[NSMutableDictionary alloc] init];
			[user_dict setObject:[a_user getStartingName] forKey:@"username"];
