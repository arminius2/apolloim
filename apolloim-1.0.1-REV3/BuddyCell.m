...
//		AlexLog(@"BuddyCell: Data Going to be Loaded");

		[self reloadData];

//		AlexLog(@"BuddyCell: Data Reloaded");

		[[[buddy getOwner] getProtocolInterface] addEventListener:self];

//		AlexLog(@"BuddyCell: initializing complete");
	}
	return self;
...
-(void)increaseUnreadCount
{
	...
	AlexLog(@"Count: %i", [buddy getMessageCount]);
...
//	AlexLog(@"Cell Updating Status Message: '%@'", [buddy getStatusMessage]);
...
-(void) respondToEvent:(Event *) event
{
	...
	&& ([[[event getOwner]getName]isEqualToString:[[buddy getOwner]getName]]))
	{
		...
		AlexLog(@"INCREASING UNREADCOUNT");
		if(![buddy isConversationVisible])
	...
	{
///		AlexLog(@"NEW STATUS MESSAGE FOR %@ THAT IS '%@'",[[event getBuddy] getName], [[event getBuddy] getStatusMessage]);
...
//		if([buddy isAway]!=[[event getBuddy]isAway])
		AlexLog(@"STATUS CHANGE FOR %@: WAS %d COULD BE %d now",[[event getBuddy] getName],[buddy isAway],[[event getBuddy] isAway]);
