...
gboolean adium_source_remove(guint tag) {
    ...
    if (sourceInfo) {
		...
		AlexLog(@"adium_source_remove(): Removing for fd %i [sourceInfo %x]: tag is %i (timer %i, read %i, write %i)",sourceInfo->fd,
...
#ifdef PURPLE_SOCKET_DEBUG
				AlexLog(@"adium_source_remove(): Done with a socket %x, so invalidating it",sourceInfo->socket);
#endif
...
#ifdef PURPLE_SOCKET_DEBUG
				AlexLog(@"adium_source_remove(): Calling updateSocketForSourceInfo(%x)",sourceInfo);
#endif				
...
{	
	if (fd < 0) {
		AlexLog(@"INVALID: fd was %i; returning tag %i",fd,sourceId+1);
		return ++sourceId;
	}
	 ...
	 */
#ifdef PURPLE_SOCKET_DEBUG
	AlexLog(@"adium_input_add(): Adding input %i on fd %i", condition, fd);
#endif
	...
	if (!(info->run_loop_source)) {
		...
		if (info->run_loop_source) {
			...
			AlexLog(@"*** Unable to create run loop source for %p",socket);
		}		
	}
						   ...
						   void *infoVoid)
{
    ...
    if ((callbackType & kCFSocketReadCallBack)) {
		if (sourceInfo->read_tag) {
			...
			AlexLog(@"Called read with no read_tag (read_tag %i write_tag %i) for %x",
		...
		if (sourceInfo->write_tag) {
			...
			AlexLog(@"Called write with no write_tag (read_tag %i write_tag %i) for %x",
				  sourceInfo->read_tag, sourceInfo->write_tag, sourceInfo->socket);
		}
	}
	...
	if (ioFunction) {
#ifdef PURPLE_SOCKET_DEBUG
		AlexLog(@"socketCallback(): Calling the ioFunction for %x, callback type %i (%s: tag is %i)",s,callbackType,
		...
		
		{
			...
			if(FD_ISSET(fd, &thisfd)){
				...
				if(length == -1)
				{
					...
					ret = -1;
					*error = ENOTCONN;
					AlexLog(@"adium_input_get_error(%i): Socket is NOT valid", fd);
				}
			}
		}
	}
...
static void buddy_event_idle_time(PurpleBuddy *buddy, gboolean old_idle, gboolean idle, PurpleBuddyEvent event)
{
	...
	AlexLog(@"BUDDY %s IDLE SINCE %@",buddy->name, [NSDate dateWithTimeIntervalSince1970:idleTime]);
...
{	
	...
	/*
	AlexLog(@"=------");
	AlexLog(@"buddy_event_status> WHO: %s",buddy->name);
	AlexLog(@"buddy_event_status> AVAILABLE: %d",isAvailable);
	AlexLog(@"buddy_event_status> MOBILE: %d",isMobile);
	AlexLog(@"buddy_event_status> MESSAGE: %@",newMessage);
	AlexLog(@"=------");
	*/
...
static void buddy_event(PurpleBuddy *buddy, PurpleBuddyEvent event)
{
	...
	//	AlexLog(@"BUDDY OWNER: %@",[buddy_owner getName]);
		...
		if(purple_buddy_get_alias(buddy))
		{
			...
			//AlexLog(@"NEW ALIAS: %@", [NSString stringWithCString:purple_buddy_get_alias(buddy)]);
		}
		else
			[b setAlias:@" "];
		...
		switch (event) 
		{
			case PURPLE_BUDDY_SIGNON: 
				//AlexLog(@"BUDDY SIGN ON -- %s", buddy->name);				
				[b setOnline:YES];
			...
			case PURPLE_BUDDY_SIGNOFF: {
				...
				AlexLog(@"BUDDY SIGN OFF-- %s", buddy->name);
				[[ApolloCore sharedInstance] buddyUpdate:b withCode:BUDDY_LOGOUT];				
				break;
			}
/*			case PURPLE_BUDDY_SIGNON_TIME: {
				AlexLog(@"BUDDY SIGN ON TIME-- %s", buddy->name);
				break;
			}
			case PURPLE_BUDDY_EVIL: 
			{
				AlexLog(@"BUDDY SIGN WARN-- %s", buddy->name);
				break;
			}*/
			case PURPLE_BUDDY_ICON: {
				AlexLog(@"BUDDY ICON RECEIVE-- %s", buddy->name);
				
				break;
			}
			/*case PURPLE_BUDDY_NAME: {
				AlexLog(@"BUDDY RENAME-- %s", buddy->name);
				break;
			}
			default: {
				AlexLog(@"BUDDY UNKNOWN EVENT");
				break;
			}*/
		}			
	}
...
{
    AlexLog(@"apollo_notify_message> Title: %s Primary: %s Secondary: %s",title,primary,secondary);
...
{		
	...
	if ((flags & PURPLE_MESSAGE_SEND) == 0) 
	{
		/*
		AlexLog(@"=------");
		AlexLog(@"apollo_receive_im> WHO: %s", who);
		AlexLog(@"apollo_receive_im> MESSAGE: %s",message);
		AlexLog(@"apollo_receive_im> TIMESTAMP: %s",purple_utf8_strftime("(%H:%M:%S)", localtime(&mtime)));
		AlexLog(@"=------");	
		*/
	@try{		
		...
		@catch (NSException* exception){
			AlexLog(@"RECEIVE IM: (%@) %@", [exception name], [exception reason]);
		}		
	}
 }
...
{
	AlexLog(@"we really don't wanna focus.");
	AlexLog(@"I DONT WANNA WORK I JUST WANNA BANG ON THE DRUM ALLLLL DAY");
	AlexLog(@"I DONT WANNA WORK I JUST WANNA BANG ON THE DRUM ALLLLL DAY");
	AlexLog(@"I DONT WANNA WORK I JUST WANNA BANG ON THE DRUM ALLLLL DAY");
	AlexLog(@"I DONT WANNA WORK I JUST WANNA BANG ON THE DRUM ALLLLL DAY");		
	return NO;
...
{
	AlexLog(@"Confirmation asked.  I don't care.");
}
...
{
    AlexLog(@"Connecting: gc=0x%x (%s) %i / %i", gc, text, step, step_count);
...
{
	AlexLog(@"Connected: gc=%x", gc);
}

static void apolloConnDisconnected(PurpleConnection *gc)
{
	AlexLog(@"apolloConnDisconnected> Forcefully Disconnected: gc=%x", gc);
//	[[ApolloCore sharedInstance] disconnected:purple_connection_get_account(gc)];
	[[ApolloCore sharedInstance] disconnect:[[ApolloCore sharedInstance] getApolloUser:purple_connection_get_account(gc)]];
...
{
	AlexLog(@"apolloConnNotice> Notice: gc=%x", gc);
    AlexLog(@"Connection Notice: gc=%x (%s)", gc, text);
}

static void apolloConnReportDisconnect(PurpleConnection *gc, const char *text)
{
	NSLog(@"JUST WORK");
	AlexLog(@"apolloConnReportDisconnect> Connection Disconnected: gc=%x (%s)", gc, text);
	[[ApolloCore sharedInstance]	connectionStatus:	-1 
									withMessage:		[NSString stringWithCString:text] 
									forAccount:			purple_connection_get_account(gc)
	];
...
static void apolloConnReportRealDisconnect()
{
	AlexLog(@"This shit is truly bananas!");
}

static void apolloConnReportRealRealDisconnect()
{
	AlexLog(@"This shit is really bananas!");
}


static PurpleConnectionUiOps apollo_conn_uiops = {
    apolloConnConnectProgress,	//1
    apolloConnConnected,		//2
    apolloConnDisconnected,		//3
    apolloConnNotice,			//4
    apolloConnReportDisconnect,	//5  -- Called when active? wtf does that mean...
	apolloConnReportRealDisconnect, //6
	apolloConnReportRealRealDisconnect
};
...
static void ui_init()
{
	...
	AlexLog(@"Initializing UI modules...");
...
//	purple_debug_set_enabled(FALSE);
	...
	purple_eventloop_set_ui_ops(&adiumEventLoopUiOps);		//Dear adium: I love you.  you make me happy inside.  this is adiums code.  not mine.  gpl4evr
	purple_dnsquery_set_ui_ops(&adiumPurpleDnsRequestOps);  //Dear adium: I love you.  you make me happy inside.  this is adiums code.  not mine.  gpl4evr
	...
	{
		AlexLog(@"libpurple initialization failed. Dumping core.\n");
		exit(-1);
	}
