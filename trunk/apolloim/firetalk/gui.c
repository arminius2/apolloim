...
void gotip (void *c, void *cs, char *ip) {
	clearline();
	printw("(%s) Got server IP: %s\n",firetalk_strprotocol(firetalk_get_protocol(c)),ip);
	printline();
}

void backlog (void *c, void *cs, double backlog) {
	clearline();
	printw("(%s) Backlog is now %fs\n",firetalk_strprotocol(firetalk_get_protocol(c)),backlog);
	printline();
}
...
void buddy_online (void *c, void *cs, const char * const who, const char * const group) {
	clearline();
	printw("(%s) '%s' of group '%s' is now online\n",firetalk_strprotocol(firetalk_get_protocol(c)),who,group);
	printline();
}

void buddy_offline (void *c, void *cs, const char * const who, const char * const group) {
	clearline();
	printw("(%s) '%s' of group '%s' is now offline\n",firetalk_strprotocol(firetalk_get_protocol(c)),who,group);
	printline();
}

void buddy_away (void *c, void *cs, const char * const who, const char * const group) {
	clearline();
	printw("(%s) '%s' of group '%s' is now away\n",firetalk_strprotocol(firetalk_get_protocol(c)),who,group);
	printline();
}

void buddy_unaway (void *c, void *cs, const char * const who, const char * const group) {
	clearline();
	printw("(%s) '%s' of group '%s' is now back\n",firetalk_strprotocol(firetalk_get_protocol(c)),who,group);
	printline();
}
...
void got_idle (void *c, void *cs, const char * const who, const char * const group, const long idletime) {
	clearline();
	printw("(%s) '%s' of group '%s' is now idle %ld minutes\n",firetalk_strprotocol(firetalk_get_protocol(c)),who,group,idletime);
	printline();
}
...
void listbuddy (void *c, void *cs, const char * const nickname, const char * const group, const int online, const int away, const long idle) {
	clearline();
	printw("(%s) Buddy: '%s' of group '%s' (online=%d, away=%d, idle=%ld)\n",firetalk_strprotocol(firetalk_get_protocol(c)),nickname,group,online,away,idle);
	printline();
}
...
void chat_joined (void *c, void *cs, const char * const room) {
	clearline();
...
int main(int argc, char *argv[]) {
	...
	for (i = 0; i < FP_MAX; i++) {
		...
		firetalk_register_callback(handles[i],FC_GOTIP,(ptrtofnct) gotip);
		firetalk_register_callback(handles[i],FC_BACKLOG,(ptrtofnct) backlog);
			...
			while ((inchar = getch()) != ERR) {
				if (inchar == '\n') {
					...
					if (!strcmp(args[0],"chat_deop")) {
						...
						if (numargs == 2) {
							i = firetalk_file_offer(handles[proto],args[1],args[2]);
							if (i != FE_SUCCESS)
								printw("Error: %s\n",firetalk_strerror(i));
						} else
...
" quit                     Exits firetalk\n");
						...
						if (numargs == 2) {
							i = firetalk_im_add_buddy(handles[proto],args[1],args[2]);
							if (i != FE_SUCCESS)
								printw("Error: %s\n",firetalk_strerror(i));
						} else
							printw("Usage: im_add_buddy <nickname> <group>\n");
						...
						if (numargs == 2) {
							i = firetalk_im_remove_buddy(handles[proto],args[1],(strcasecmp(args[2],"null") == 0) ? NULL : args[2]);
							if (i != FE_SUCCESS)
								printw("Error: %s\n",firetalk_strerror(i));
						} else
							printw("Usage: im_remove_buddy <nickname> <group>\n");
