...
-(id) initWithFrame:(CGRect) aframe
{
	if ((self == [super initWithFrame: aframe]) != nil) 
	{
		...
		AlexLog(@"Buddy View Init Complete");
	}
	return self;
}
...
- (void)tableRowSelected:(NSNotification *)notification
{
	...
	AlexLog(@"%@ clicked, Transitioning", [b getName]);
...
- (void)reloadData
{
	...
	for(i; i<[users count]; i++)
	{
		...
		if([u isActive])
		{
			...
			//AlexLog(@"%@ contain %i buddies.", [u getName], [b count]);
			...
			for(j; j<[b count]; j++)
			{
				...
				//AlexLog(@"%@ isOnline=%d conversationExists=%d", [buddy getName], [buddy isOnline], [[ViewController sharedInstance] conversationWithBuddyExists: buddy]);
